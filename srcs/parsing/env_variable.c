...
/*   Updated: 2023/10/05 01:59:06 by edelarbr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**take_exitcode(t_shell_memory *data, char **line, char **var)
{
	...
	exitcode[1] = ft_memset(ft_calloc(data, sizeof(char), (ft_strlen(exitcode[0])
...
char	**take_after(t_shell_memory *data, char **line)
{
	...
	if (after == NULL)
		return (NULL);
	after[0] = ft_substr(data, line[0], there_is_a_env_var(line) + env_var_len(line)
			+ 1, 2147483647);
	after[1] = ft_substr(data, line[1], there_is_a_env_var(line) + env_var_len(line)
			+ 1, 2147483647);
	return (after);
}
