...
/*   Updated: 2023/09/17 22:18:35 by edelarbr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*take_var(char **line)
{
	...
	while (line[0][++start])
		if (line[0][start] == '$' && ft_isenvchar(line[0][start + 1])
			&& line[1][start] == '0' && line[1][start + 1] == '0')
			break ;
	if (line[0][start] == '$')
		start++;
	len = 0;
	while (ft_isenvchar(line[0][start + len]) && line[1][start + len] == '0')
		len++;
	var = ft_substr(line[0], start, len);
	printf("var: \"%s\"\n", var);
	return (var);
}
...
char	**take_replacement(char *var, char **env)
{
	...
	while (env[++i])
	{
		if (ft_strncmp(joined, env[i], ft_strlen(joined)) == 0)
		{
			...
			replacement[1] = ft_memset(malloc(sizeof(char) * (ft_strlen(replacement[0]) + 1)), '0', ft_strlen(replacement[0]));
...
char	**take_after(char **line)
{
	...
	if (line[0][start] == '$')
		start += ft_strlen(take_var(line)) + 1;
