...
/*   Updated: 2023/10/07 20:32:15 by edelarbr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (input_line[0][++i])
	{
				...
				|| input_line[0][i] == ')'))
		{
			return (ft_putstr_fd(
				...
				ft_putchar_fd(input_line[0][i], 2), ft_putstr_fd("'\n", 2),
				data->exit_code = 258);
		}
		if ((input_line[1][i] == '0' && input_line[0][i] == '&'
			...
			return (ft_putstr_fd(
					"minishell: syntax error near unexpected token `", 2),
				ft_putchar_fd(input_line[0][i], 2), ft_putchar_fd(
				input_line[0][i + 1], 2), ft_putstr_fd("'\n", 2),
				data->exit_code = 258);
	}
	return (0);
}
