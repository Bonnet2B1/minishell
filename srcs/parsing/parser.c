...
/*   Updated: 2023/10/06 01:34:05 by edelarbr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	parsing(t_shell_memory *data)
{
	if (is_empty_or_whitespaces(data->input_line[0]))
		return (0);
	if (!quotes_gestion(data, data->input_line))
		return (0);
	env_var_gestion(data, data->input_line);
	if (forbiddens_chars(data, data->input_line))
		return (0);
	crazy_split(data, data->input_line);
	tokenization(data->parsing_lst);
	rm_quotes(data, data->parsing_lst);
