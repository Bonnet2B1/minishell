...
/*   Updated: 2023/09/25 16:59:06 by edelarbr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	setup_fd(t_shell_memory *data, t_list *exec_lst)
{
	...
	while (exec_lst)
	{
		...
		if (((t_exec *)exec_lst->content)->in_struct
			&& (((t_exec *)exec_lst->content)->in_struct->token == PIPE))
			((t_exec *)exec_lst->content)->in_fd = ((t_exec *)exec_lst->content)->in_struct->pipe_fd[OUTPIPE];

		if (((t_exec *)exec_lst->content)->out_struct
			&& (((t_exec *)exec_lst->content)->out_struct->token == PIPE))
			((t_exec *)exec_lst->content)->out_fd = ((t_exec *)exec_lst->content)->out_struct->pipe_fd[INPIPE];
